...
keywords: >-
  requisitos, dnf, instalação, rhel, rpm, instalar, instalar o Docker Engine,
  desinstalar, atualizar
title: Instale a Docker Engine no RHEL
linkTitle: RHEL
weight: 30
toc_max: 4
aliases:
...
Para começar a usar a Docker Engine no RHEL, certifique-se de que você
...
1. Instale os pacotes do Docker.
   ...
   sua versão do RHEL (indicada pelo sufixo `.el9` neste exemplo).
...
2. Selecione sua versão do RHEL na lista.

3. Selecione a arquitetura aplicável (`x86_64`, `aarch64` ou `s390x`) e em
   seguida, vá para `stable/Packages/`.

4. Baixe os seguintes arquivos `rpm` para a Docker Engine, CLI, containerd e
   pacotes do Docker Compose:

   - `containerd.io-<versao>.<arquitetura>.rpm`
   - `docker-ce-<versao>.<arquitetura>.rpm`
   - `docker-ce-cli-<versao>.<arquitetura>.rpm`
   - `docker-buildx-plugin-<versao>.<arquitetura>.rpm`
   - `docker-compose-plugin-<versao>.<arquitetura>.rpm`

5. Instale a Docker Engine, alterando o seguinte caminho para o caminho onde
   você baixou os pacotes do Docker.

   ```console
   $ sudo dnf install ./containerd.io-<versao>.<arquitetura>.rpm \
     ./docker-ce-<versao>.<arquitetura>.rpm \
     ./docker-ce-cli-<versao>.<arquitetura>.rpm \
     ./docker-buildx-plugin-<versao>.<arquitetura>.rpm \
     ./docker-compose-plugin-<versao>.<arquitetura>.rpm
...
7. Verifique se a instalação foi bem-sucedida executando a imagem `hello-world`:
