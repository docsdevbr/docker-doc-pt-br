...
- The [`kubernetes` driver](builders/drivers/kubernetes.md) now supports a `timeout` option for configuring deployment timeout. [docker/buildx#2492](https://github.com/docker/buildx/pull/2492/index.md)
...
- The [`buildx prune`](../../reference/cli/docker/buildx/prune.md) and [`buildx du`](../../reference/cli/docker/buildx/du.md) commands now support negative and prefix filters. [docker/buildx#2473](https://github.com/docker/buildx/pull/2473/index.md)
...
- Add `requests.ephemeral-storage`, `limits.ephemeral-storage` and `schedulername` options to the [`kubernetes` driver](builders/drivers/kubernetes.md).
...
- Bake now supports [matrix builds](bake/reference.md#targetmatrix).
...
- The `buildx build` command supports new `--attest` flag, along with
  ...
  - `--attest type=sbom` or `--sbom=true` adds [SBOM attestations](metadata/attestations/sbom.md).
  - `--attest type=provenance` or `--provenance=true` adds [SLSA provenance attestation](metadata/attestations/slsa-provenance.md).
...
- Adding extra hosts with [Docker driver](builders/drivers/docker.md) now supports
...
- Support for a new [`remote` driver](builders/drivers/remote.md) that you can use
